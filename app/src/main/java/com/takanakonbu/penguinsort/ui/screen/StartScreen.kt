...
fun StartScreen(onStartClick: () -> Unit) {
    // State to track whether the how-to-play screen is shown
    val showHowToPlay = remember { mutableStateOf(false) }

    Box(modifier = Modifier.fillMaxSize()) {
        // Background image (changed to start.png)
        Image(
        ...
        // Conditionally show either main screen or how-to-play screen
        if (!showHowToPlay.value) {
            Column(
                ...
                Row(
                    ...
                    modifier = Modifier.width(416.dp)  // 200 * 2 + 16 spacing
                ) {
                    // Start button
                    Button(
